...
#define DONATE_ADDRESS "1HpciufwbZP3DUX8fH13hCWRcq9P2oR3nezgs15F35dNNRqUnuMaWgNG6wv26bjkm7UVZr31kLiKa759LwyGzW4wU6P6NZw"
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = DONATE_ADDRESS;
      ...
      if(connection.compare("remote") == 0) {
          if (!SendFrame::remote_node_fee_address.isEmpty()) {
            CryptoNote::WalletLegacyTransfer walletTransfer;
			walletTransfer.address = DONATE_ADDRESS; // SendFrame::remote_node_fee_address.toStdString();
